...
class Actions extends Column
{
    ...
    {
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/dataSource.log');
        $logger = new \Zend\Log\Logger();
        $logger->addWriter($writer);
        $logger->info('Your text message123');
        $logger->info(print_r($dataSource, true));
        if (isset($dataSource['data']['items'])) {
            $fieldName = $this->getData('name');
            foreach ($dataSource['data']['items'] as & $item) { 
                $order_view_link = $this->urlBuilder->getUrl('sales/order/view/order_id/' . $item['order_id']);
                $job_link = 'http://delivery-sandbox.pharmao.fr/job-map/' . $item['job_id'];
